  ...
  if (db.getAllUsers().length === 0) {
    ...
    const [tweet1, tweet2, tweet3, tweet4] = [
      await db.createTweet({
        userId: fem.id,
        message:
          'Hey, check this out! https://www.youtube.com/embed/mi5F8x5K9iE',
      }),
      await db.createTweet({
        userId: marc.id,
        message:
          "Check out Mary's story -- new on the QVC youtube channel! https://www.youtube.com/embed/2Ij4QQCx-gc",
      }),
      await db.createTweet({
        userId: lisa.id,
        message:
