...
async function seedDb(db) {
    if (db.getAllTrends().length === 0) {
        ...
        await db.createTopicTrend({ topic: 'Frontend Masters', tweetCount: 12345 }, {
            description: 'Launch of new full stack TS course',
            imageUrl: 'http://localhost:3000/static/fem_logo.png',
            title: 'Frontend Masters',
        });
        await db.createTopicTrend({ topic: 'Pointless Suggestions', tweetCount: 999 }, {
    ...
    if (db.getAllUsers().length === 0) {
        ...
        const [tweet1, tweet2, tweet3, tweet4] = [
            await db.createTweet({
                userId: fem.id,
                message: 'Hey, check this out! https://www.youtube.com/embed/mi5F8x5K9iE',
            }),
            await db.createTweet({
                userId: marc.id,
                message: "Check out Mary's story -- new on the QVC youtube channel! https://www.youtube.com/embed/2Ij4QQCx-gc",
            }),
            await db.createTweet({
                userId: lisa.id,
                message: "@1Marc I'm having trouble logging into FEM. This is the captcha I'm being asked to solve. Seems a bit challenging http://localhost:3000/static/captcha-1.jpg ",
            }),
            await db.createTweet({
                userId: marc.id,
